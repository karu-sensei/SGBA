...
class Formularios extends Form implements InputFilterAwareInterface
{
        ...
        if(!$this->inputfilter){
            ...
            $inputfilter->add($factory->createInput(array(
                ...
                'validators'=>array(array(
                    ...
                    'name' => 'Identical',
                    'options' => array(
                        'token' => 'confirmar' //I have tried $_POST['password'], but it doesnt work either
                    )
                )
            )
						)));
	...
	public function validaedita()
    {
       

        if(!$this->inputfilter2){
            $inputfilter2= new InputFilter();
            $factory2 = new InputFactory();
            //$dbAdapter=$this->getServiceLocator()->get('Zend\Db\Adapter');
			
				
            $inputfilter2->add($factory2->createInput(array(
                'name'=>'nombre',
                'required'=>'true',
                'filter'=>array(array(
                'name'=>'HtmlEntities',
        ...
        $this->inputfilter2= $inputfilter2;
        return $this->inputfilter2;
